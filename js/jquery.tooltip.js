(function($){
	...
	$.fn.tooltip = function(instanceSettings){
		...
		var showTooltip = function(){
			...
			$('#' + s.tooltipID + ' #tooltipContent').on('click', function () {
		...
		return this.each(function(index){
			...
			function on(e){	
			    $('body').append('<div id="'+s.tooltipID+'" style="background-repeat:no-repeat;background-image:url('+s.tooltipBGImage+');padding:'+s.tooltipPadding+'px;display:none;height:'+s.height+';width:'+s.width+';background-color:'+s.tooltipBGColor+';border:'+s.borderSize+'px solid '+s.borderColor+'; border-radius:'+s.borderRadius+'; position:absolute;z-index:100000000000; font-size: 12px;"><div id="tooltipContent" style="display:none;"></div></div>');
